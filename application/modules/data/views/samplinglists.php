   <?php
    ...
    if($k_tpi != ""){
     ...
     if($k_tpi != '')
     {
     		...
     		$query = "SELECT * from master_landing where id_landing ='".$mk_tpi."'";
			$result = $this->db->query($query)->row_array();
			$nama_landing = $result['nama_landing'] ; 
...
<table border="1" width="100%" class="table-style">
  ...
  if ($mbulan=='xxx')
  {
    ...
    $query = $query." from tps_pendaratan,tab_bulan where k_landing='".$mk_tpi."' and thn_sampling=".$mtahun.' and tab_bulan.k_bulan=tps_pendaratan.bln_sampling'." and status_trip = '1' ";

    if(count($user->list_landing) > 0){

   	$addition = " and tps_pendaratan.k_landing in ( ";
  ...
  else
  {
    ...
    $query = $query." from tps_pendaratan where k_landing='".$mk_tpi."' and thn_sampling=".$mtahun.' and bln_sampling='.$mbulan." and status_trip = '1' ";
	...
	<tr>
		<?php
          if ($mbulan=='xxx')
          {
    		...
    		$sql=$sql." and k_landing='".$mk_tpi."' and thn_sampling=".$mtahun." and bln_sampling=".$record->bln_sampling." and status_trip = '1' ";
    		   		if(count($user->list_landing) > 0){
          ...
          {
    		$sql="select sum(tps_bycatch.berat) as berat from tps_bycatch,tps_pendaratan where tps_bycatch.namafile=tps_pendaratan.namafile";
  		    $sql=$sql." and k_landing='".$mk_tpi."' and thn_sampling=".$mtahun." and bln_sampling=".$mbulan." and tgl_sampling=".$record->tgl_sampling." and status_trip = '1'";
  		    if(count($user->list_landing) > 0){
          ...
          if ($mbulan=='xxx')
          {
          	...
          	<td align="center" ><a href="<?php echo base_url()."data/mainpage/samplinglists/".$mk_tpi."/".$mtahun."/".$record->bln_sampling; ?>"><?php echo $record->n_bulan; ?></a></td>
		...
		<?php
          ...
          {
          	?>
           	<td align="center"><a href="<?php echo base_url()."data/mainpage/samplinglists_tgl/".$mk_tpi."/".$mtahun."/".$mbulan."/".$record->tgl_sampling; ?>"><?php echo $record->tgl_sampling; ?></a></td>
...
<?php 
  ...
  $query = "select tps_pendaratan.k_landing,nama_landing,fake,thn_sampling,sum(bbm) as bbm, sum(es) as es,";
  $query = $query."count(master_landing.id_landing) as landing,sum(total_penangkapan) as tangkapan,sum(est_ikanhilang) as hilang, sum(lama_hari) as lama_hari, sum(lama_jam) as lama_jam ";
  $query = $query."from tps_pendaratan, master_landing where master_landing.id_landing=tps_pendaratan.k_landing and status_trip = '1' ";
  ...
  $query = $query."group by tps_pendaratan.k_landing,nama_landing,fake,thn_sampling order by tps_pendaratan.k_landing,nama_landing,thn_sampling ";
...
?>
					...
					<table border="1" width="100%" class="table-style">
						...
						<tr>
							<td align="center" bgcolor="#CCCCCC"><b>Lokasi</b></td>
							...
							<td align="center" bgcolor="#CCCCCC"><b>Jenis II</b></td>	
												...
												<?php 
												  ...
												  {
													//for rowspan
													$query = "select count(distinct(thn_sampling)) as hasil from tps_pendaratan where k_landing = '".$record->k_landing."' and status_trip = '1'" ; 
													if(count($user->list_landing) > 0){
													...
													$nama_lokasi = '';
													  ...
													  {
														$sw=1;
														$mn_tpi=$record->nama_landing;
														$teks=$record->nama_landing;
															
															$nama_lokasi = '<td align="center" rowspan = "'.$result_years->hasil.'" ><center>'.$teks.'</center></td>';
													  }
													  elseif ($mn_tpi!=$record->nama_landing )
													  {
														$mn_tpi=$record->nama_landing;
												...
												?>
													<tr>
														<?php
												  		  $sql="select sum(tps_bycatch.berat) as berat from tps_bycatch,tps_pendaratan where tps_bycatch.namafile=tps_pendaratan.namafile";
												  		  $sql=$sql." and k_landing='".$record->k_landing."' and thn_sampling=".$record->thn_sampling." and status_trip = '1' ";
												  		  if(count($user->list_landing) > 0){
												    ...
												    $addition .=" ) ";				
												   	...
												   	$sql = $sql." ".$addition;
												          ...
												          else
												          {
												  		  ...
												  		  $sql="select sum(tps_ikanbesar.berat) as berat from tps_ikanbesar,tps_pendaratan where tps_ikanbesar.namafile=tps_pendaratan.namafile";
												  		  $sql=$sql." and k_landing='".$record->k_landing."' and thn_sampling=".$record->thn_sampling." ";
												  		  if(count($user->list_landing) > 0){
														...
														<td align="center" ><a href="<?php echo base_url()."data/mainpage/samplinglists/".$record->k_landing."/".$record->thn_sampling; ?>"><?php echo $record->thn_sampling; ?></a></td>
														<td align="center"><?php echo number_format($record->landing); ?></td>
														<td align="center"><?php echo number_format($record->lama_hari); ?></td>
														<td align="center"><?php echo number_format($record->lama_jam); ?></td>
														<td align="center"><?php echo number_format($record->bbm); ?></td>
...
<?php 
  ...
  $query = $query."from tps_pendaratan, master_supplier where master_supplier.id_supplier=tps_pendaratan.k_perusahaan and status_trip = '1' ";
  												if(count($user->list_supp) > 0){

												   	$addition = " and tps_pendaratan.k_perusahaan in ( ";
												    ...
												    $addition .=" ) ";				
												   	...
												   	$sql = $sql." ".$addition;
												          ...
												          else
												          {
												  		  ...
												  		  $sql="select sum(tps_ikanbesar.berat) as berat from tps_ikanbesar,tps_pendaratan where tps_ikanbesar.namafile=tps_pendaratan.namafile";
												  		  $sql=$sql." and k_perusahaan='".$record->k_perusahaan."' and thn_sampling=".$record->thn_sampling." ";
